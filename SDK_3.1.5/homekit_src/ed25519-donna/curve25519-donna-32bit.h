...
_attribute_ram_code_ void curve25519_copy(bignum25519 out, const bignum25519 in) {
	for (int i=0; i<10; i++)
		{
			out[i] = in[i];
		}
...
static void
_attribute_ram_code_ curve25519_add(bignum25519 out, const bignum25519 a, const bignum25519 b) {
	for (int i=0; i<10; i++)
	{
		out[i] = a[i] + b[i];
	}
...
_attribute_ram_code_ void curve25519_add_after_basic(bignum25519 out, const bignum25519 a, const bignum25519 b) {
	uint32_t c = 0;
	for (int i=0; i<10; i++)
	{
		out[i] = a[i] + b[i] + c; c = out[i] >> 26; out[i] &= reduce_mask_26;
		i++;
		out[i] = a[i] + b[i] + c; c = out[i] >> 25; out[i] &= reduce_mask_25;
	}
	out[0] += 19 * c;
}

_attribute_ram_code_ void curve25519_add_reduce(bignum25519 out, const bignum25519 a, const bignum25519 b) {
	curve25519_add_after_basic (out, a, b);
}
...
curve25519_sub_after_basic(bignum25519 out, const bignum25519 a, const bignum25519 b) {
	...
	for (int i=0; i<10; i++)
	{
		out[i] = p     + a[i] - b[i] + c    ; c = (out[i] >> 26); out[i] &= reduce_mask_26;
		p = fourP13579;
		i++;
		out[i] = p     + a[i] - b[i] + c    ; c = (out[i] >> 25); out[i] &= reduce_mask_25;
		p = fourP2468;
	}
	out[0] += 19 * c;
...
curve25519_neg(bignum25519 out, const bignum25519 a) {
	...
	for (int i=0; i<10; i++)
	{
		out[i] = p     - a[i] + c    ; c = (out[i] >> 26); out[i] &= reduce_mask_26;
		p = twoP13579;
		i++;
		out[i] = p     - a[i] + c    ; c = (out[i] >> 25); out[i] &= reduce_mask_25;
		p = twoP2468;
	}
	out[0] += 19 * c;
#endif
...
#if 0
void
_attribute_ram_code_ curve25519_mul(bignum25519 out, const bignum25519 a, const bignum25519 b) {
...
#else
static void curve25519_mul_carry (bignum25519 output, u64 * m)
{
	u64 c = 0;
	u32 p;
	for (int i=0; i<10; i++)
	{
		m[i] += c;
        output[i] = (uint32_t)m[i] & reduce_mask_26;
        c = (m[i] >> 26);

        i++;
        m[i] += c;
        output[i] = (uint32_t)m[i] & reduce_mask_25;
        c = (m[i] >> 25);
	}
	p = c;
	c = output[0] + mul32x32_64(p,19);
	output[0] = (uint32_t)c & reduce_mask_26;
	p = (uint32_t)(c >> 26);
	output[1] += p;
}

static void curve25519_mul(bignum25519 output, const bignum25519 in2, const bignum25519 in)
{
	uint64_t m[10];
	uint32_t r[10];
	...
	for (int o=0; o<10; o++)
	{
		...
		int a = 0;
		do {
			u64 ma = mul32x32_64 (r[a], in2[o-a]);
			if (m2 & a & 1)
			{
				ma <<= 1;
			}
...
static inline void  curve25519_square(bignum25519 output, const bignum25519 in) {
	//curve25519_mul (output, in, in);
	//return;
	uint64_t m[10];
	uint32_t r[10];
...
static void curve25519_expand(bignum25519 out, const unsigned char in[32]) {
	u32 x[10];
	u8 sft[10] = {32, 26, 19, 13, 6, 32, 25, 19, 12, 6};
	x[0] = x[5] = 0;
	memcpy (x + 1, in, 16);
	memcpy (x + 6, in + 16, 16);
	...
	{
		u32 a = *p++;
		out[i] = ((((uint64_t)p[0] << 32)  | a ) >> sft[i]) & 0x3ffffff;
		i++;
		a = *p++;
		out[i] = ((((uint64_t)p[0] << 32)  | a ) >> sft[i]) & 0x1ffffff;
	}
}
#endif
...
curve25519_contract(unsigned char out[32], const bignum25519 in) {
	...
	#define F(i, s) \
		out[s+0] |= (unsigned char )(f[i]); \
		out[s+1] = (unsigned char )((f[i] >> 8)); \
		out[s+2] = (unsigned char )((f[i] >> 16)); \
		out[s+3] = (unsigned char )((f[i] >> 24));
...
curve25519_swap_conditional(bignum25519 a, bignum25519 b, uint32_t iswap) {
	...
	for (int i=0; i<10; i++)
	{
		u32 x = swap & (a[i] ^ b[i]); a[i] ^= x; b[i] ^= x;
	}
}
#endif
