...
static uint16_t m_adv_interval_ms = 200;
...
static mible_status_t mible_service_data_set(mibeacon_config_t const * const config,
        uint8_t *p_output, uint8_t *p_output_len)
{
    uint32_t errno;
    uint8_t data_len = 0;
...
mible_status_t mibeacon_init(uint8_t key[16], bool is_newone)
{
...
mible_status_t mibeacon_adv_data_set(bool is_solicited, uint8_t mesh_stat, uint8_t *p_user_data, uint8_t user_dlen)
{
    mibeacon_frame_ctrl_t fctrl = {
